...
from utils import plot_distribution_stacked_bar, plot_stacked_bars_original_states, plot_stacked_bars_combined_states, plot_fold_distribution_stacked_bar, plot_learning_curves, plot_metric_learning_curves, plot_roc_curves, calculate_metrics
...
CONFIG = {
    ...
    "results_dir": "results_21062025_01",
    "data_fraction": 1.0, 
    "alpha": 0.3,
    "temperature": 5.0,
    "AdamW": True
...
def run_kfold_experiment(model_name, model, train_loader, val_loader, fold_idx, is_distillation=False, teacher_model=None):
    ...
    for epoch in range(start_epoch, CONFIG['num_epochs']):
        ...
        # El scheduler ajustará el LR basándose en la pérdida de validación
...
if __name__ == '__main__':
    ...
    dataloaders_kfold, df_per_fold = create_train_dataloaders(CONFIG['data_path'], CONFIG['k_folds'], CONFIG['batch_size'], CONFIG['data_fraction'])
    all_fold_results = {"Teacher": [], "Student_Baseline": [], "Student_KD": []}


    train_FileName = os.path.join(CONFIG['data_path'], 'train.csv')
    train_df = pd.read_csv(os.path.join(CONFIG['data_path'], 'train.csv'))
    print(f"Loaded {len(train_df)} rows from File 1. {train_FileName}")
    val_FileName = os.path.join(CONFIG['data_path'], 'valid.csv')
    val_df = pd.read_csv(os.path.join(CONFIG['data_path'], 'valid.csv'))
    print(f"Loaded {len(val_df)} rows from File 1. {val_FileName}")
   
    train_dataset = CheXpertDataset(train_df, transform=get_transforms(is_train=True), is_train=True)
    val_dataset = CheXpertDataset(val_df, transform=get_transforms(is_train=False), is_train=True)
    ...
    plot_stacked_bars_original_states(train_df, "Train Set", TARGET_CLASSES_5, CONFIG['results_dir'])

    plot_stacked_bars_combined_states(train_df, "Train Set", TARGET_CLASSES_5, CONFIG['results_dir'])
    

    plot_stacked_bars_original_states(val_df, "Validation Set", TARGET_CLASSES_5, CONFIG['results_dir'])

    plot_stacked_bars_combined_states(val_df, "Validation Set", TARGET_CLASSES_5, CONFIG['results_dir'])
    
    # for fold_idx, (train_df, val_df) in enumerate(df_per_fold):

    #     print(f"Loaded {len(train_df)} rows from Train Set  Fold {fold_idx + 1}. ")
    
    #     plot_stacked_bars_original_states(train_df, f"Fold {fold_idx + 1} Train Set", TARGET_CLASSES_5, CONFIG['results_dir'])

    #     plot_stacked_bars_combined_states(train_df, f"Fold {fold_idx + 1} Train Set", TARGET_CLASSES_5, CONFIG['results_dir'])
        
    #     print(f"Loaded {len(val_df)} rows from Testing Set  Fold {fold_idx + 1}. ")
    
    #     plot_stacked_bars_original_states(val_df, f"Fold {fold_idx + 1} Validation Set", TARGET_CLASSES_5, CONFIG['results_dir'])

    #     plot_stacked_bars_combined_states(val_df, f"Fold {fold_idx + 1} Validation Set", TARGET_CLASSES_5, CONFIG['results_dir'])
